...
public class UserDaoImpl implements UserDao
{
	...
	public void register(String username,String password,double money) {
		...
		if(prop == null){
			...
			throw new UserExitException("该用户已经存在，注册不成功！");
		}
	} 
	...
	public User login(String username,String password){
		...
		if(prop != null) {
			if(password.equals(prop.getProperty("password"))){
				...
				throw new PasswordErrorException("用户名密码错误！");
			}
		} else {
			throw new UserNotExitException("用户名不存在！");
		}
		return user;
	}
   ...
   public void checkBalance(User user) {
	...
	public void transfer(User user,String name,double money) {
		...
		if(prop1 != null) {
			...
			throw new  UserNotExitException("您要转的账户不存在!");
